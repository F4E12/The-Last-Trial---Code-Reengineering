package id.ac.binus.solutionOOP.config;

import id.ac.binus.solutionOOP.core.animations.IAnimation;
import id.ac.binus.solutionOOP.core.models.Vector2D;
import id.ac.binus.solutionOOP.controllers.Direction;
import javafx.scene.canvas.GraphicsContext;

public class DefaultRenderer implements Renderer {
    private final Direction direction;

    public DefaultRenderer(Direction direction) {
        this.direction = direction;
    }

    @Override
    public void render(GraphicsContext gc, IAnimation animation, Vector2D pos) {
        if (animation == null) return;

        int deltaX = animation.getCropWidth();
        int deltaY = animation.getCropHeight();
        int startX = animation.getCurrentFrame() * deltaX;
        int startY = 0;
        int adjustedWidth = deltaX;

        if (direction == Direction.LEFT) {
            startX += deltaX;
            adjustedWidth = -deltaX;
        }

        gc.drawImage(animation.getSpriteImage(), startX, startY, adjustedWidth, deltaY,
                pos.getX(), pos.getY() - deltaY * 4, deltaX * 4, deltaY * 4);
    }
}